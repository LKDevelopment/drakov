        ...
        drafter.parse(data, options, function (err, result) {
            ...
            function setupResourceAndUrl(resource) {
                ...
                resource.actions.forEach(function (action) {
                    if (action.attributes.uriTemplate.indexOf('actions/') != -1) {
                        var _parsedUrl = urlParser.parse(action.attributes.uriTemplate);
                        var _key = _parsedUrl.url;
                        routeMap[_key] = routeMap[_key] || {urlExpression: _key, methods: {}};
                        parseAction(_parsedUrl, action, routeMap);
                        saveRouteToTheList(_parsedUrl, action);
                    } else {
